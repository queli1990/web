...
// 横
class HorizontaLists extends Component {
  renderHoriLists () {
    ...
    for (let i = 0; i < lists.data.length; i++) {
      ...
      let item = (
        <figure key={i} onClick={()=>{this.playVideo(i)}}>
          <img src={a.landscape_poster_s} alt="not found"/>
          <p className="figure-name">{a[name]}</p>
          <div className={a.pay ? 'horvip': ''}></div>
        </figure>
      )
      arr.push(item)
    }
