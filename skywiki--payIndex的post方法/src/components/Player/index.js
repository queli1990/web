...
export default class Player extends Component {
  ...
  postRecord () {
    if(this.adFlag) { return this.adFlag = false }
  ...
  playADs () {
    ...
    let SpotX = window.SpotX
    if ( SpotX === undefined || tools.isVip()) return
  ...
  render() {
    // if (window.location.pathname === "/") return (<div></div>)
  ...
  episodes () {
    if (!(this.typeId === 3)){ //电影
      ...
      for (var i = 0; i < this.state.episodes.length; i++) {
        ...
        let index = i;
        let evip = i >= this.state.episodes.length - this.album.pay_eps ? 'evip' : '';

        // let url = item.download[0].link
        array.push(
          <a key={i} className={currentName} onClick={()=>this.chooseEpisode(index, evip)}>{title}
          <div className={evip}></div>
          </a>
        )
      }
      return array
    ...
    for (var i = 0; i < this.state.related.length; i++) {
      ...
      tempArray.push(
        <figure className="cell" key={i} onClick={()=>this.selecteRelated(index)}>
          ...
          <div className={item.pay ? 'rvip' : ''}></div>
        </figure>
      )
    }
    return tempArray;
  ...
  chooseEpisode (index, str) {
    ...
    if (tools.isVip() || str === '') {
      ...
      if(!tools.isLogin()) {
        return this.refs.header.showLoginView();
      } else {
