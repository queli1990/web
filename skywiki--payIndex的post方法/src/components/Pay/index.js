...
class Pay extends Component {
  constructor () {
    ...
    this.changePayWayLink = this.changePayWayLink.bind(this)
    this.checkOrderIsPay = this.checkOrderIsPay.bind(this)
    this.cheakPay = this.cheakPay.bind(this)
  }
  ...
  fetchPayLink(i) {
    ...
    let platform = 'web-pc';
    let channel = 'uu100';
    let combinStr = this.state.userInfoDic.token+packageId+paywayId+platform+channel;
    let md5Str = md5(combinStr);
    let url = `http://api.100uu.tv/app/order/doPayment.do?token=${this.state.userInfoDic.token}&packageId=${packageId}&paywayId=${paywayId}&platform=${platform}&channel=${channel}&sign=${md5Str}`
  ...
  changePayWayLink() {
    ...
    let paywayId = this.state.payWays[this.state.payWaysSelected-1].payId;
    let combinStr = md5(this.state.userInfoDic.token + orderNo + this.state.payWaysSelected + 'web');
    
    let url = `http://api.100uu.tv/app/order/doRepay.do?token=${this.state.userInfoDic.token}&orderNo=${orderNo}&paywayId=${paywayId}&platform=web&sign=${combinStr}`
    let _this = this;
    axios(url)
    .then((res)=>{
  ...
  userInfoView() {
    ...
    let name = this.state.userInfoDic.userName;
    let dueTime = this.state.userInfoDic.dueTime;
  ...
  checkOrderIsPay() {
    ...
    this.timeInterval = window.setInterval(() => this.cheakPay(), 5000);
  }
  cheakPay() {
    let orderNo ;
    if (this.state.packagesSelected == 1) {
    ...
    let url = `http://api.100uu.tv/app/order/doQueryOrderForPC.do`
    let data = {token: this.state.userInfoDic.token, orderNo :orderNo, platform:'web', channel:'uu100'}
    let _this = this
    axios({
      method: 'post',
      url: 'http://api.100uu.tv/app/order/doQueryOrderForPC.do',
      params: data,
    })
    .then((res)=>{
